...
// --- Data (Readme Content) ---
// En una aplicación real, esto podría venir de un archivo .md importado o una API
const readmeContent = `
...
const parseReadmeToSections = (markdown: string): Section[] => {
    ...
    lines.forEach(line => {
        ...
        if (newTitle) {
            ...
            currentId = newTitle.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]+/g, '');
            currentSectionLines = [];
        } else {
...
const App: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (parsed.length > 0 && parsed[0].title === "DashMaster - Task Management System" && parsed[0].level === 1) {
            setSections(parsed.slice(1));
        } else {
            setSections(parsed);
        }
    ...
    const welcomeSection: Section = useMemo(() => ({
        ...
        level: 0,
        content: `DashMaster es una aplicación integral de gestión de tareas diseñada para ayudar a los equipos a organizar su trabajo, realizar un seguimiento del progreso y visualizar métricas de rendimiento. 
    La aplicación integra la planificación de sprints, la gestión de equipos, los indicadores clave de rendimiento (KPI) y las capacidades de generación de informes para proporcionar una solución completa para los equipos ágiles modernos.
    ...
    return (
        <div className="flex flex-col min-h-screen bg-slate-100 text-slate-800">
            <div className="flex flex-1"> {/* Contenedor para Sidebar y Main Content */}
                <button
            ...
            <Footer /> {/* Footer añadido aquí, fuera del flex-1 principal */}
        </div>
    );
};
